# 🦀 Portafolio de Oscar - Rust Static Site

![Rust](https://img.shields.io/badge/Made_with-Rust-orange?style=for-the-badge&logo=rust)
![Status](https://img.shields.io/badge/Status-En_Producción-success?style=for-the-badge)
![License](https://img.shields.io/badge/License-MIT-blue?style=for-the-badge)

¡Bienvenido a mi portafolio personal! Este proyecto no es solo una muestra de mi trabajo, sino también una demostración de desarrollo web moderno utilizando **Rust** para generar un sitio estático seguro, rápido y eficiente.

Puedes ver el sitio en vivo aquí: 

## 🚀 Tecnologías Utilizadas

* **Lenguaje Principal:** [Rust](https://www.rust-lang.org/) 🦀

## 🛠️ Instalación y Ejecución Local

Si deseas clonar y correr este portafolio en tu máquina local:

1.  **Clonar el repositorio:**
    ```bash
    git clone [https://github.com/Oscar-CR09/Nombre-Del-Repo.git](https://github.com/Oscar-CR09/Nombre-Del-Repo.git)
    cd Nombre-Del-Repo
    ```

2.  **Instalar dependencias (ejemplo con Cargo):**
    ```bash
    cargo build --release
    ```

3.  **Correr el servidor local:**
    ```bash
    # Ejemplo si usas una herramienta como Trunk o Zola
    cargo run
    # O
    trunk serve
    ```

4.  Abre tu navegador en `http://127.0.0.1:8080`

## 📂 Estructura del Proyecto

* `src/`: Código fuente en Rust.
* `static/`: Archivos estáticos (imágenes, CSS).
* `dist/` o `public/`: El sitio compilado listo para producción.

---
_Desarrollado con ❤️ y mucho código en Rust._
